...
export default function VideoAnalysis() {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      setUploadStatus('');
      setStreamSource(`http://127.0.0.1:8000/video/stream_file?filename=${encodeURIComponent(data.filename)}`);
    } catch (error) {
      console.error(error);
  ...
  const startCamera = () => {
    setStreamSource('http://127.0.0.1:8000/video/stream_camera');
  };

  const startUrlStream = () => {
    if (url) {
      setStreamSource(`http://127.0.0.1:8000/video/stream_url?url=${encodeURIComponent(url)}`);
    }
  };
